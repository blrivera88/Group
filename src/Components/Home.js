...
function Home() {
  ...
  const [isEditMode, setIsEditMode] = useState(false); // State to track if Edit mode is active
  ...
  const handleMovieSelect = (movie) => {
    ...
    setMovies([]); // Clear the search results after selection
  };
  ...
  const toggleEditMode = () => {
    setIsEditMode((prevIsEditMode) => !prevIsEditMode); // Toggle Edit mode
  };
